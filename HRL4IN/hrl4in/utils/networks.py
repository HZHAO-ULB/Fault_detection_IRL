...
class Net(nn.Module):
    ...
    def __init__(self,
        ...
        super().__init__()
        #print("this observation spaces ",observation_space.spaces)
        ...
        if "task_obs" in observation_space.spaces:
            self._n_task_info = observation_space.spaces["task_obs"].shape[0]
        else:
            self._n_task_info = 0
        ...
        #print('additional rnn input num: ', self._n_additional_rnn_input)
    ...
    def forward_perception_model(self, observations):
        ...
        #print("cnn inpiut dim: ", cnn_input.size())
    ...
    def forward(self, observations, rnn_hidden_states, masks):
        if self._n_additional_rnn_input > 0:
            ...
            if self._n_task_info >0:
                additional_rnn_input.append(torch.unsqueeze(observations["task_obs"],2))
            ...
            x = torch.squeeze(torch.cat(additional_rnn_input, dim=1),-1)
