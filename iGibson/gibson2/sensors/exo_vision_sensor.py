...
class ExoVisionSensor(BaseSensor):
    ...
    def get_obs(self, env):
        ...
        env.simulator.renderer.set_camera(
                self.camerapos, self.camerapos + self.view_direction, self.up)

        raw_vision_obs = env.simulator.renderer.render(modes=self.raw_modalities)
        ...
        if 'rgb' in self.modalities:
            vision_obs['exo_rgb'] = self.get_rgb(raw_vision_obs)
        if 'rgb_filled' in self.modalities:
            vision_obs['exo_rgb_filled'] = self.get_rgb_filled(raw_vision_obs)
        if 'depth' in self.modalities:
            vision_obs['exo_depth'] = self.get_depth(raw_vision_obs)
        if 'pc' in self.modalities:
            vision_obs['exo_pc'] = self.get_pc(raw_vision_obs)
        if 'optical_flow' in self.modalities:
            vision_obs['exo_optical_flow'] = self.get_optical_flow(raw_vision_obs)
        if 'scene_flow' in self.modalities:
            vision_obs['exo_scene_flow'] = self.get_scene_flow(raw_vision_obs)
        if 'normal' in self.modalities:
            vision_obs['exo_normal'] = self.get_normal(raw_vision_obs)
        if 'seg' in self.modalities:
            vision_obs['exo_seg'] = self.get_seg(raw_vision_obs)
        return vision_obs
