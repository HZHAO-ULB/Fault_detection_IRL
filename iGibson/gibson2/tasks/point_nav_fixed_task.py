...
from gibson2.termination_conditions.door_angle import DoorAngle
from gibson2.termination_conditions.robot_death import robotDeath
...
from gibson2.reward_functions.door_status_reward import DoorStatusReward
from gibson2.reward_functions.door_angle_reward import DoorAngleGoalReward
...
class PointNavFixedTask(BaseTask):
    ...
    def __init__(self, env):
        ...
        self.termination_conditions = [
            ...
            DoorAngle(self.config),
            robotDeath(self.config),
        ]
        self.reward_functions = [
            #PotentialReward(self.config),
            CollisionReward(self.config),
            #PointGoalReward(self.config),
            DoorStatusReward(self.config),
            DoorAngleGoalReward(self.config),
        ]
    ...
    def get_door_handler_distance(self, env):
        """
        Compute task-specific potential: distance to the door handle

        :param env: environment instance
        :return: task potential
        """
        return np.linalg.norm(
            np.array(p.getLinkState(env.door.body_id, env.door_handle_link_id)[0]) -
            np.array(p.getLinkState(env.robots[0].robot_ids[0], env.jr_end_effector_link_id)[0])
        )
    ...
    def get_do_task_obs(self, env):
        ...
        :return: task-specific observation
        """
        #3D xyz relative coordonate from robot to door handle
        task_obs = self.global_to_local(env, np.array(p.getLinkState(env.door.body_id, env.door_handle_link_id)[0]))
        end_effector_pos = self.global_to_local(env, np.array(p.getLinkState(env.robots[0].robot_ids[0], env.jr_end_effector_link_id)[0]))
        door_angle = p.getJointState(env.door.body_id, env.door_axis_link_id)[0]
        cos_door_angle, sin_door_angle = np.cos(door_angle), np.sin(door_angle)
        has_door_handle_in_hand = 1.0 if env.stage == env.stage_open_door else -1.0
        #action mask state
        action_state = 1.0
        # linear velocity along the x-axis
        linear_velocity = rotate_vector_3d(
        ...
        angular_velocity = rotate_vector_3d(
            ...
            *env.robots[0].get_rpy())[2]
        task_obs = np.append(
            task_obs, [action_state, has_door_handle_in_hand, linear_velocity, angular_velocity, door_angle, cos_door_angle, sin_door_angle])
        task_obs = np.append(
            task_obs, end_effector_pos
        )
        task_obs = np.append(
            task_obs, env.robots[0].get_position()
        )

        return task_obs

    def get_do_task_obs_low(self, env):
        """
        Get task-specific observation, including goal position, current velocities, etc.
        ...
        has_door_handle_in_hand = 1.0 if env.stage == env.stage_open_door else -1.0
        # action mask state
        #action_state = env.action_mask_state
        action_state = 1.0
        # linear velocity along the x-axis
        linear_velocity = rotate_vector_3d(
            env.robots[0].get_linear_velocity(),
        ...
        task_obs = np.append(
            task_obs,
            [action_state, has_door_handle_in_hand, linear_velocity, angular_velocity, door_angle, cos_door_angle,
             sin_door_angle])
        task_obs = np.append(
            task_obs, end_effector_pos
        )
    ...
    def get_do_fdd_obs(self, env):
        ...
        """
        #3D xyz relative coordonate from robot to door handle
        fdd_obs = self.global_to_local(env, np.array(p.getLinkState(env.door.body_id, env.door_handle_link_id)[0]))
        end_effector_pos = self.global_to_local(env, np.array(p.getLinkState(env.robots[0].robot_ids[0], env.jr_end_effector_link_id)[0]))
        fdd_obs = np.append(
            fdd_obs, end_effector_pos)
        # 3D xyz global coordonate of robot and door handle
        fdd_obs = np.append(fdd_obs,
                            np.array(p.getLinkState(env.door.body_id, env.door_handle_link_id)[0]))
        fdd_obs = np.append(fdd_obs,
                            np.array(p.getLinkState(env.robots[0].robot_ids[0], env.jr_end_effector_link_id)[0]))
